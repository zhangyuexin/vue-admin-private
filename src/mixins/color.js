...
export default function() {
  return {
    ...
    mounted() {
      ...
      this.updateTheme(this.themeVal, this.themeVal);
    },
    watch: {
      themeVal(val, oldVal) {
    ...
    methods: {
      updateTheme(val, oldVal) {
        ...
        const getHandler = (variable, id) => {
          return () => {
            ...
            // console.log(this.updateStyleColor);
            const newStyle = this.updateStyleColor(
              this[variable],
              originalCluster,
              themeCluster
            );
        ...
        for (
          let i = link.length - 2 >= 0 ? link.length - 2 : 0;
          i < link.length;
          i++
        ) {
          const style = link[i];
        ...
        styles.forEach(style => {
          ...
          style.innerText = this.updateStyleColor(
            innerText,
            originalCluster,
            themeCluster
          );
      ...
      updateStyleColor(style, oldCluster, newCluster) {
        let newStyle = style;
        // console .log(
        //   style,
        //   oldCluster,
